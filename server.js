...
// 🔑 তোমার দেওয়া Gemini API Key সরাসরি এখানে সেট করে দেওয়া হলো
    ...
    if (data.type === 'chat') {
      try {
        if (!GEMINI_API_KEY || GEMINI_API_KEY.includes("YOUR_API_KEY")) {
          socket.emit('ai response', { type: 'chat', text: "API Key সেটআপ করা হয়নি। দয়া করে server.js ফাইলে আপনার Gemini API Key যুক্ত করুন।" });
          return;
        }
